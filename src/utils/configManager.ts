...
export class ConfigManager {
    ...
    /**
     * Get Claude invocation mode (cli or extension)
     */
    getClaudeInvocationMode(): 'cli' | 'extension' {
        return this.settings?.claude?.invocationMode || 'cli';
    }
    ...
    async saveSettings(settings: PrismaSettings): Promise<void> {
        if (!this.workspaceFolder) {
            throw new Error('No workspace folder found');
        }
