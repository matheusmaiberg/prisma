...
export class UpdateChecker {
    private static readonly SKIP_VERSION_KEY = 'prisma.skipVersion';
    private static readonly LAST_CHECK_KEY = 'prisma.lastUpdateCheck';
    ...
    private getCurrentVersion(): string | undefined {
        const extension = vscode.extensions.getExtension('matheusmaiberg.prisma-for-cc');
    ...
    private async fetchLatestRelease(): Promise<any> {
        try {
            ...
            const response = await fetch('https://api.github.com/repos/matheusmaiberg/prisma/releases/latest');
    ...
    private showUpdateNotification(currentVersion: string, latestVersion: string): void {
        const message = `🎉 Prisma ${latestVersion} is available! (current: ${currentVersion})`;
        ...
        vscode.window.showInformationMessage(
            ...
            if (selection === 'View Changelog') {
                ...
                const releaseUrl = 'https://github.com/matheusmaiberg/prisma/releases/latest';
