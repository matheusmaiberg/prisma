        ...
        if (!element) {
            ...
            if (this.isLoading) {
                ...
                items.push(new AgentItem(
                    getTranslations().agents.loadingAgents,
            ...
            items.push(new AgentItem(
                getTranslations().agents.userAgents,
            ...
            if (projectAgents.length > 0 || vscode.workspace.workspaceFolders) {
                items.push(new AgentItem(
                    getTranslations().agents.projectAgents,
                    vscode.TreeItemCollapsibleState.Expanded,
                    'agent-group',
                    'project'
                ));
...
class AgentItem extends vscode.TreeItem {
    constructor(
        ...
        if (contextValue === 'agent-loading') {
            ...
            this.tooltip = getTranslations().agents.loadingAgents;
        } else if (contextValue === 'agent-group') {
            // Use icons similar to Steering Explorer
            if (groupType === 'user') {
                this.iconPath = new vscode.ThemeIcon('globe');
                this.tooltip = getTranslations().agents.userAgentsTooltip;
            } else {
                this.iconPath = new vscode.ThemeIcon('root-folder');
                this.tooltip = getTranslations().agents.projectAgentsTooltip;
            }
