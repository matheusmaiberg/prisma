...
This is a VSCode extension called "Prisma for Claude Code" that enhances Claude Code with structured spec-driven development features. The extension provides visual management of specs (requirements, design, tasks) and steering documents.
...
- **Command Registration**: All commands are registered in `activate()` with pattern `prisma.{feature}.{action}`
